...
DATA_FILE = "updated_oih_database_25_aug.json"
API_URL = os.getenv("UPDATED_API")
UPDATE_INTERVAL = 60 * 60  # 20 hour in seconds
...
#     """Create the SQL generation prompt template (token-optimized)"""
#     return PromptTemplate(
#         input_variables=["user_input", "columns", "sample_data"],
#         template="""
# You are an expert in **pandasql + SQLite**. Work on a DataFrame named `df`.

# ### Table Info
# - Columns: {columns}
# - Sample (3 rows): {sample_data}

# ### Rules
# 1. SQL must be SQLite-compatible.  
# 2. Use `WhereCondData` for dates (default). Use `ShippedDate` only if when user ask for shippedvalue or total shipped value.  
# 3. **Total turnover** = `SUM(InHandValue + ShippedValueInUSD)`.  
# 4. Show month names (e.g., "January") not numbers.  
# 5. Filters must be case-insensitive.  
# 6. Use proper `GROUP BY` with aggregates.  
...
- Use Tolerance column for date it is our main date, and second main date is ShippedDate.
...
def main():
    ...
    # Page configuration
    st.set_page_config(
        page_title="Talk to Database Updated 25-8-2025",
        page_icon="🗣️",
        layout="wide"
    )
    
    st.title("🗣️ Talk to Database Updated 25-8-2025")
